...
export interface GenerateContentResult {
  result: any;
  source: 'gemini' | 'smart-template' | 'fallback';
  message?: string;
}
...
export async function handleGenerateContent(
  ...
  if (!ai) {
    ...
    return { result: fallback, source: 'smart-template' };
  }

  let prompt = '';
  const systemInstruction = `You are the Lead Creative Director & Social Media Strategist for "Tatna Crocheting Corner", a luxury handmade crochet brand based in Marondera, Zimbabwe.
...
Format as JSON:
{
  ...
  "schedule": [
    ...
    return { result: parsedResult, source: 'gemini' };
  } catch (error: any) {
    console.error('Gemini content generation error:', error);
    // Graceful fallback on any API error
    const fallback = generateFallbackContent(type, product, options);
    return { result: fallback, source: 'fallback', message: error?.message };
  }
}
...
function generateFallbackContent(type: string, product: any, options: any) {
  ...
  return {
    ...
    schedule: [
      { day: 1, dayName: 'Monday', pillar: 'Behind The Scenes', postType: 'Reels / Video (9:16)', productFocus: 'Slouchy Beanie or Top', contentIdea: 'Macro ASMR video of yarn needle and tension check', captionHook: 'Starting the week with fresh yarn and endless possibilities 🧶✨', bestPostingTime: '12:30 PM & 6:00 PM CAT' },
      { day: 2, dayName: 'Tuesday', pillar: 'Styling Inspiration', postType: 'Carousel Post (1:1 / 4:5)', productFocus: 'Crochet Slops & Bags', contentIdea: '3 ways to style handmade crochet footwear and tote bags', captionHook: 'Which colorway is your favorite? 1, 2, or 3? 👇', bestPostingTime: '1:00 PM & 7:00 PM CAT' },
      { day: 3, dayName: 'Wednesday', pillar: 'Product Spotlight', postType: 'Story Series + WhatsApp Status', productFocus: 'Sunflower & Rose Bouquets', contentIdea: 'Everlasting flowers that never wilt - price breakdown from $1 USD', captionHook: 'Real flowers fade in a week. These blooms last forever 🌻💕', bestPostingTime: '11:00 AM & 5:30 PM CAT' },
      { day: 4, dayName: 'Thursday', pillar: 'Customer Proof & Packing', postType: 'Short Video / Reel', productFocus: 'Custom Order Delivery', contentIdea: 'Pack an order with me: folding, tag attaching, and wrapping', captionHook: 'Packing this beauty heading to its new home today! 📦✨', bestPostingTime: '2:00 PM & 6:30 PM CAT' },
      { day: 5, dayName: 'Friday', pillar: 'Weekend Outfit Drop', postType: 'High-Res Photo & Status Card', productFocus: 'Bralette / Crop Top Collection', contentIdea: 'Golden hour model shoot or clean flatlay with gold jewelry', captionHook: 'Weekend plans sorted with the perfect handmade piece ✨', bestPostingTime: '3:30 PM & 7:30 PM CAT' },
      { day: 6, dayName: 'Saturday', pillar: 'Engagement / Poll', postType: 'Interactive Story Polls', productFocus: 'New Yarn Color Swatches', contentIdea: 'Help me choose next week yarn palette (Sage Green vs Butter Yellow)', captionHook: 'Vote on our story: which shade should we make next? 🎨', bestPostingTime: '10:00 AM & 4:00 PM CAT' },
      { day: 7, dayName: 'Sunday', pillar: 'Weekly Slot Booking Reminder', postType: 'Official Price List Poster Export', productFocus: 'Full Menu & Custom Orders', contentIdea: 'Official weekly menu recap: prices from $1 USD with direct WhatsApp link', captionHook: 'Booking slots for next week are now open! Reserve yours early 🗓️', bestPostingTime: '5:00 PM & 8:00 PM CAT' }
    ]
  };
  }
